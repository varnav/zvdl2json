...
while True:
    ...
    # Dump all non-empty ACARS messages to CSV and .txt
    if 'acars' in data['vdl2']['avlc']:
    ...
    if 'acars' in data['vdl2']['avlc']:
        regex = r'K\d\d\d\d | OIL'
        if bool(re.search(regex, data['vdl2']['avlc']['acars']['msg_text'])):
            print('Possible flight plan!')
            print(data['vdl2']['avlc']['acars']['msg_text'])
            filename = homedir + datetime.datetime.now().strftime("%Y-%m-%d_%h") + '_finds.json'
            with open(filename, 'a', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=4)
        regex = r'[S,N] \d\d\.\d\d [W,E] \d\d\d\.\d\d | FPO\/\d\d'
        if bool(re.search(regex, data['vdl2']['avlc']['acars']['msg_text'])):
